...
export const MealPatternView = () => {
  ...
  const [sel, setSel] = useState<Record<string, string>>({ entree: "", grain: "", fruit: "", vegetable: "", milk: "1% Low-Fat Milk" });
  const [grade, setGrade] = useState("K-5");
  const byC = useMemo(() => { const m: Record<string, Recipe[]> = {}; recipes.forEach(r => { if (!m[r.category]) m[r.category] = []; m[r.category].push(r); }); return m; }, [recipes]);
  const limits = USDA_LIMITS[grade];
  const selected = useMemo(() => {
    const i: { nutrition: Recipe["nutrition"] }[] = [];
    if (sel.entree) { const r = recipes.find(r => r.name === sel.entree); if (r) i.push(r); }
    if (sel.grain) { const r = recipes.find(r => r.name === sel.grain); if (r) i.push(r); }
    if (sel.fruit) { const r = recipes.find(r => r.name === sel.fruit); if (r) i.push(r); }
    if (sel.vegetable) { const r = recipes.find(r => r.name === sel.vegetable); if (r) i.push(r); }
    if (sel.milk) i.push(MILK_ITEM);
    return i;
  }, [sel, recipes]);
